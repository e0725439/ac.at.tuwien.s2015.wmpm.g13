...
public class JettyRouteBuilder extends RouteBuilder {
	...
	public void configure() {
		...
		restConfiguration().component("jetty").host("localhost").port(8181).bindingMode(RestBindingMode.auto);
		...
		from("direct:order_put").process(new Processor() {
			@Override
			public void process(Exchange exchange) throws Exception {
				SimpleOrder body = exchange.getIn().getBody(SimpleOrder.class);
		...
		rest("/services/rest").get("/test/simpleorder").produces("application/json")
		.to("direct:generate_restorder");
